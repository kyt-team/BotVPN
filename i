...
const MERCHANT_CODE = "T39266";          // Merchant Code Tripay
const API_KEY = "bT2qnA6GZrL944JvLYxIp9nQOXY0ilfkE5w3jqfl";       // API Key Tripay
const PRIVATE_KEY = "fX6iK-Hslsb-OIzTS-M7MN6-5GHgL";      // Private Key Tripay
const TRIPAY_URL = "https://tripay.co.id/api-sandbox/transaction/create"; // sandbox
const CALLBACK_URL = "https://159.223.53.83/tripay/callback";
...
let userToday = 0, userWeek = 0, userMonth = 0, userTotal = 0;
let globalToday = 0, globalWeek = 0, globalMonth = 0, globalTotal = 0;

try {
  // Statistik User
  userToday = await new Promise((resolve) => {
    db.get(
      'SELECT COUNT(*) as count FROM transactions WHERE user_id = ? AND timestamp >= ? AND type IN ("ssh","vmess","vless","trojan","shadowsocks")',
      [userId, todayStart],
  ...
  userTotal = await new Promise((resolve) => {
    db.get(
      'SELECT COUNT(*) as count FROM transactions WHERE user_id = ? AND type IN ("ssh","vmess","vless","trojan","shadowsocks")',
      [userId],
      (err, row) => resolve(row ? row.count : 0)
    );
  });
  ...
  globalToday = await new Promise((resolve) => {
    db.get(
      'SELECT COUNT(*) as count FROM transactions WHERE timestamp >= ? AND type IN ("ssh","vmess","vless","trojan","shadowsocks")',
      [todayStart],
      (err, row) => resolve(row ? row.count : 0)
    );
  });
  ...
  globalTotal = await new Promise((resolve) => {
    db.get(
      'SELECT COUNT(*) as count FROM transactions WHERE type IN ("ssh","vmess","vless","trojan","shadowsocks")',
      (err, row) => resolve(row ? row.count : 0)
    );
  });
} catch (e) {
...
async function sendAdminMenu(ctx) {
  const adminKeyboard = [
    ...
    [
      ...
      { text: '💵 lihat Saldo', callback_data: 'viewsaldo_user' },
    ],
    [
...
// Menu Awal - Lihat Saldo
bot.action('viewsaldo_user', async (ctx) => {
  try {
    logger.info('View saldo user process started');
    await ctx.answerCbQuery();

    const users = await new Promise((resolve, reject) => {
      db.all('SELECT user_id FROM users LIMIT 2000', [], (err, users) => {
        if (err) {
    ...
    if (totalUsers > 20) {
      replyMarkup.inline_keyboard.push([{
        text: '➡️ Next',
        callback_data: `next_users_view_${currentPage + 1}`
      }]);
    }

    await ctx.reply('👀 *Silakan pilih user untuk melihat saldo:*', {
      reply_markup: replyMarkup,
      parse_mode: 'Markdown'
    });
  } catch (error) {
    logger.error('❌ Kesalahan saat memulai proses lihat saldo user:', error);
    await ctx.reply(`❌ *${error}*`, { parse_mode: 'Markdown' });
  }
});

// Handler klik user -> tampilkan saldo
bot.action(/view_saldo_(.+)/, async (ctx) => {
    ...
    const saldo = await new Promise((resolve, reject) => {
      db.get('SELECT saldo FROM users WHERE user_id = ?', [userId], (err, row) => {
        if (err) {
          ...
          return reject('⚠️ *PERHATIAN! Terjadi kesalahan saat mengambil saldo user.*');
        }
        resolve(row ? row.saldo : null);
      });
    });

    if (saldo === null) {
      await ctx.reply(`⚠️ *User ${userId} tidak ditemukan.*`, { parse_mode: 'Markdown' });
    } else {
      await ctx.reply(`💰 Saldo user *${userId}*: Rp ${saldo.toLocaleString()}`, { parse_mode: 'Markdown' });
    }
  } catch (error) {
    logger.error('❌ Kesalahan saat melihat saldo user:', error);
    await ctx.reply(`❌ *${error}*`, { parse_mode: 'Markdown' });
  }
});
...
// === Fungsi bikin transaksi Tripay ===
async function createTripayPayment(userId, amount) {
  const merchantRef = `TOPUP-${userId}-${Date.now()}`;

  const payload = {
    method: "QRIS", // Bisa ganti: BRIVA, BNIVA, MANDIRIVA, dll
    merchant_ref: merchantRef,
    amount: amount,
    customer_name: `User ${userId}`,
    customer_email: `user${userId}@mail.com`,
    ...
    return_url: "https://159.223.53.83/thanks",
    expired_time: Math.floor(Date.now() / 1000) + 300, // 5 menit expired
  };
  ...
  const res = await axios.post(TRIPAY_URL, payload, { headers });
  return res.data;
}
...
async function processDeposit(ctx, amount) {
  try {
    ...
    db.run("INSERT OR IGNORE INTO users (user_id, nama, saldo) VALUES (?, ?, 0)", [
      userId,
      ctx.from.first_name,
    ]);

    // bikin transaksi tripay
    const tripay = await createTripayPayment(userId, amount);

    if (tripay.success) {
      const data = tripay.data;

      db.run(
        "INSERT INTO pending_deposits (user_id, reference, amount, status) VALUES (?, ?, ?, ?)",
        [userId, data.reference, amount, "PENDING"]
      );
      ...
      await ctx.reply(
        `💳 *Topup Saldo*\n\n` +
          ...
          `Ref: ${data.reference}\n\n` +
          `Silakan bayar pakai QRIS / link berikut:\n\n` +
          `${data.payment_url}\n\n` +
          `⌛ Berlaku 5 menit.`,
        { parse_mode: "Markdown" }
      );
    } else {
      await ctx.reply("❌ Gagal membuat transaksi Tripay.");
    }
  } catch (err) {
    console.error(err);
    await ctx.reply("⚠️ Error saat membuat transaksi deposit.");
  }
}

// === Command /topup ===
bot.command("topup", async (ctx) => {
  await ctx.reply("💰 Masukkan jumlah topup saldo (contoh: 50000)");

  bot.on("text", async (ctx2) => {
    const amount = parseInt(ctx2.message.text);
    if (isNaN(amount) || amount < 10000) {
      return ctx2.reply("❌ Minimal topup Rp10.000");
    }
    await processDeposit(ctx2, amount);
  });
});
...
bot.command("saldo", (ctx) => {
  ...
  db.get("SELECT saldo FROM users WHERE user_id = ?", [userId], (err, row) => {
    if (row) {
      ctx.reply(`💰 Saldo Anda: Rp${row.saldo.toLocaleString()}`);
    } else {
      ctx.reply("⚠️ Anda belum punya akun.");
    }
  });
});

// === CALLBACK TRIPAY ===
app.post("/tripay/callback", express.json(), (req, res) => {
  const { reference, status } = req.body;

  if (status === "PAID") {
    db.get(
      "SELECT user_id, amount FROM pending_deposits WHERE reference = ?",
      [reference],
      (err, row) => {
        if (row) {
          ...
          db.run("DELETE FROM pending_deposits WHERE reference = ?", [reference]);

          bot.telegram.sendMessage(
            row.user_id,
            `✅ Topup Rp${row.amount.toLocaleString()} berhasil masuk saldo!`
          );
        }
      }
    );
